...
test("anchors the purple center and left measurements to line h, not the green inset", () => {
  // These angles keep line h's endpoint inside the parcel while covering both
  ...
  for (const angle of [EXAMPLE_ANGLE, 86.89, PRESET_ANGLES.reverse, 110.23]) {
    ...
    // The center-50 line ends where the left-15 line begins: line h's end.
...
test("squares a 15 ft strip off each side, ends cut at a right angle", () => {
  ...
  assert.notEqual(areas.strips.left.y, areas.strips.right.y);
...
test("the fitted center reaches into the independently measured left strip", () => {
  // Square on, 15 + 50 + 15 fits exactly and no ground is claimed twice.
  const square = calculateRightAngleAreas(PRESET_ANGLES.rightAngle);
